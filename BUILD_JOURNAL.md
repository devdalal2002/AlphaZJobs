...
## What I learned
AI-generated claims about "what's currently in the codebase" cannot be trusted without verification, even when they're confident and detailed. The safest workflow was always to read the actual current files before acting on any description of their state — this caught multiple mega-prompts that would have silently regressed working features if applied at face value.
...
- Actually enforce the minor-safety protections that are currently only described in Settings copy (e.g., a real gate before a minor can apply to an "18+" job).
