...
- **Built for the fact that many users are minors.** Age shown as a range instead of an exact number for under-18 users, restricted DMs (only mentors/verified employers can message minors directly), and employer mode is unavailable to minors entirely. LinkedIn has no equivalent protections because it was never built for a userbase that includes 14-year-olds.
